...
cleaned_data %>% 
  select(where(is.numeric)) %>%
  summarize(across(where(is.numeric), function(x) qqplot(x) + qqline(x)))

cleaned_data %>% 
  select(Category, where(is.numeric)) %>%
  summarize(across(where(is.numeric), function(x) qqplot(x ~ Category, col = colors)))
```

3) Ниже напишите, какие ещё методы проверки на нормальность вы знаете и какие у них есть ограничения.

a) Построение гистограммы. Если данные примерно повторяют форму кривой колокола на гистограмме, мы можем предположить, что набор данных имеет нормальное распределение.
